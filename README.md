...
* **Alya Version:** 0.0.8 (Compiled with `alyac build` in Release mode)
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| `Algorithms` | **Recursive Fibonacci** | `fib(30)` (~2.69M calls) | `2.4 ms` | **`9.7 ms`** | `13.8 ms` | `126.5 ms` | **4.1x** | **13.1x faster** | **1.4x faster** |
| `Algorithms` | **In-Place Quicksort** | 50,000 items in-place sort | `16.6 ms` | **`133.1 ms`** | `34.1 ms` | `2092.3 ms` | **8.0x** | **15.7x faster** | `3.9x slower` |
| `Algorithms` | **Sieve of Eratosthenes** | Primes under 50,000 | `1.1 ms` | **`1.7 ms`** | `7.0 ms` | `19.1 ms` | **1.5x** | **11.6x faster** | **4.2x faster** |
| `Collections` | **Binary Trees** | Heap tree allocation & traversal | `117.1 ms` | **`480.1 ms`** | `99.6 ms` | `2879.5 ms` | **4.1x** | **6.0x faster** | `4.8x slower` |
| `Collections` | **Hash Map** | 20k insertions, updates & lookups | `4.1 ms` | **`10.8 ms`** | `15.5 ms` | `26.5 ms` | **2.6x** | **2.5x faster** | **1.4x faster** |
| `Numeric` | **Mandelbrot Fractal** | 200×100 grid, 200 iters | `3.5 ms` | **`8.9 ms`** | `10.2 ms` | `134.2 ms` | **2.6x** | **15.1x faster** | **1.1x faster** |
| `Numeric` | **Matrix Multiply** | 120×120 dense integer matrix mult | `1.3 ms` | **`8.3 ms`** | `12.3 ms` | `235.6 ms` | **6.5x** | **28.5x faster** | **1.5x faster** |
| `Strings` | **FNV-1a String Hash** | 50,000 hash calculations | `5.2 ms` | **`7.9 ms`** | `14.0 ms` | `469.5 ms` | **1.5x** | **59.3x faster** | **1.8x faster** |
...
* **Result:** **4.1x of C (-O2)**, **1.4x faster than Bun**, and **13.1x faster than Python**.
...
* **Result:** **2.6x of C (-O2)**, **1.1x faster than Bun**, and **15.1x faster than Python**.
...
* **Result:** **1.5x of C (-O2)**, **4.2x faster than Bun**, and **11.6x faster than Python**.
...
* **Result:** **1.5x of C (-O2)**, **1.8x faster than Bun**, and **59.3x faster than Python**.
...
* **Result:** **4.1x of C (-O2)**, **4.8x slower than Bun**, and **6.0x faster than Python**.
...
* **Result:** **6.5x of C (-O2)**, **1.5x faster than Bun**, and **28.5x faster than Python**.
...
* **Result:** **2.6x of C (-O2)**, **1.4x faster than Bun**, and **2.5x faster than Python**.

...
| :--- | :---: | :---: | :---: | :---: |
| **Geometric Mean Relative Speed** | **1.0x (Baseline)** | `0.38x` *(faster)* | `0.69x` | `0.07x` *(slower)* |
| **Alya Relative Performance** | **Reference Target** | **~2.6x of C** | **1.4x faster** | **14.2x faster** |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| `Algorithms` | **Recursive Fibonacci** | `fib(30)` (~2.69M calls) | `2.3 ms` | **`8.8 ms`** | `12.7 ms` | `120.7 ms` |
| `Algorithms` | **In-Place Quicksort** | 50,000 items in-place sort | `14.6 ms` | **`130.7 ms`** | `32.1 ms` | `1807.5 ms` |
| `Algorithms` | **Sieve of Eratosthenes** | Primes under 50,000 | `1.0 ms` | **`1.8 ms`** | `6.6 ms` | `16.7 ms` |
| `Algorithms` | **Collatz Conjecture** | Under 100k (~2.16M steps) | `14.5 ms` | **`58.5 ms`** | `41.1 ms` | `728.1 ms` |
| `Algorithms` | **Binary Search** | 100k items, 50k lookups | `3.6 ms` | **`7.0 ms`** | `14.1 ms` | `114.9 ms` |
| `Collections` | **Binary Trees** | Heap tree allocation & traversal | `125.9 ms` | **`399.1 ms`** | `96.0 ms` | `2691.4 ms` |
| `Collections` | **Hash Map** | 20k insertions, updates & lookups | `4.3 ms` | **`10.5 ms`** | `14.2 ms` | `23.2 ms` |
| `Numeric` | **Mandelbrot Fractal** | 200×100 grid, 200 iters | `3.1 ms` | **`12.8 ms`** | `9.3 ms` | `121.0 ms` |
| `Numeric` | **Matrix Multiply** | 120×120 dense integer matrix mult | `1.2 ms` | **`6.6 ms`** | `12.0 ms` | `190.2 ms` |
| `Numeric` | **Monte Carlo Simulation** | 500,000 iterations (Pi approx) | `3.8 ms` | **`6.7 ms`** | `55.0 ms` | `191.7 ms` |
| `Strings` | **FNV-1a String Hash** | 50,000 hash calculations | `4.7 ms` | **`8.8 ms`** | `12.5 ms` | `430.4 ms` |
| `Memory` | **Linked List Allocation** | 50k node alloc & traversal | `2.9 ms` | **`2.8 ms`** | `7.1 ms` | `28.8 ms` |
| `Crypto` | **RC4 Stream Cipher** | 100k bytes KSA & PRGA stream | `1.1 ms` | **`2.0 ms`** | `6.7 ms` | `34.1 ms` |
| `Bitwise` | **Kernighan Popcount** | 100k integers bit-clearing | `2.3 ms` | **`3.8 ms`** | `8.2 ms` | `139.4 ms` |
...
| :--- | :---: | :---: | :---: | :---: |
| **Recursive Fibonacci** | **13.8x faster** | **1.4x faster** | **3.8x** | 🟢 Native Fast |
| **In-Place Quicksort** | **13.8x faster** | `4.1x slower` | **9.0x** | 🟡 Near-Native |
| **Sieve of Eratosthenes** | **9.3x faster** | **3.7x faster** | **1.8x** | 🟢 Near-C |
| **Collatz Conjecture** | **12.4x faster** | `1.4x slower` | **4.0x** | 🟢 Native Fast |
| **Binary Search** | **16.3x faster** | **2.0x faster** | **2.0x** | 🟢 Near-C |
| **Binary Trees** | **6.7x faster** | `4.2x slower` | **3.2x** | 🟢 Native Fast |
| **Hash Map** | **2.2x faster** | **1.3x faster** | **2.4x** | 🟢 Native Fast |
| **Mandelbrot Fractal** | **9.5x faster** | `1.4x slower` | **4.1x** | 🟢 Native Fast |
| **Matrix Multiply** | **29.0x faster** | **1.8x faster** | **5.3x** | 🟡 Near-Native |
| **Monte Carlo Simulation** | **28.5x faster** | **8.2x faster** | **1.8x** | 🟢 Near-C |
| **FNV-1a String Hash** | **48.6x faster** | **1.4x faster** | **1.9x** | 🟢 Near-C |
| **Linked List Allocation** | **10.1x faster** | **2.5x faster** | **1.0x** | 🟢 Near-C |
| **RC4 Stream Cipher** | **17.4x faster** | **3.4x faster** | **1.8x** | 🟢 Near-C |
| **Kernighan Popcount** | **37.0x faster** | **2.2x faster** | **1.6x** | 🟢 Near-C |
...
* **Result:** **3.8x of C (-O2)**, **1.4x faster than Bun**, and **13.8x faster than Python**.
...
* **Result:** **9.0x of C (-O2)**, **4.1x slower than Bun**, and **13.8x faster than Python**.
...
* **Result:** **1.8x of C (-O2)**, **3.7x faster than Bun**, and **9.3x faster than Python**.
...
* **Result:** **4.0x of C (-O2)**, **1.4x slower than Bun**, and **12.4x faster than Python**.
...
* **Result:** **2.0x of C (-O2)**, **2.0x faster than Bun**, and **16.3x faster than Python**.
...
* **Result:** **3.2x of C (-O2)**, **4.2x slower than Bun**, and **6.7x faster than Python**.
...
* **Result:** **2.4x of C (-O2)**, **1.3x faster than Bun**, and **2.2x faster than Python**.
...
* **Result:** **4.1x of C (-O2)**, **1.4x slower than Bun**, and **9.5x faster than Python**.
...
* **Result:** **5.3x of C (-O2)**, **1.8x faster than Bun**, and **29.0x faster than Python**.
...
* **Result:** **1.8x of C (-O2)**, **8.2x faster than Bun**, and **28.5x faster than Python**.
...
* **Result:** **1.9x of C (-O2)**, **1.4x faster than Bun**, and **48.6x faster than Python**.
...
* **Result:** **1.0x of C (-O2)**, **2.5x faster than Bun**, and **10.1x faster than Python**.
...
* **Result:** **1.8x of C (-O2)**, **3.4x faster than Bun**, and **17.4x faster than Python**.
...
* **Result:** **1.6x of C (-O2)**, **2.2x faster than Bun**, and **37.0x faster than Python**.

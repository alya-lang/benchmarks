...
| :--- | :---: | :---: | :---: | :---: |
| **Geometric Mean Relative Speed** | **1.0x (Baseline)** | `0.40x` *(faster)* | `0.64x` | `0.08x` *(slower)* |
| **Alya Relative Performance** | **Reference Target** | **~2.5x of C** | **1.6x faster** | **12.9x faster** |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| `Algorithms` | **Recursive Fibonacci** | `fib(30)` (~2.69M calls) | `1.6 ms` | **`4.3 ms`** | `8.6 ms` | `69.3 ms` |
| `Algorithms` | **In-Place Quicksort** | 50,000 items in-place sort | `9.9 ms` | **`70.5 ms`** | `22.7 ms` | `992.9 ms` |
| `Algorithms` | **Sieve of Eratosthenes** | Primes under 50,000 | `0.7 ms` | **`1.1 ms`** | `4.2 ms` | `11.4 ms` |
| `Algorithms` | **Collatz Conjecture** | Under 100k (~2.16M steps) | `14.1 ms` | **`70.4 ms`** | `33.8 ms` | `454.9 ms` |
| `Algorithms` | **Binary Search** | 100k items, 50k lookups | `3.2 ms` | **`6.5 ms`** | `12.2 ms` | `72.4 ms` |
| `Collections` | **Binary Trees** | Heap tree allocation & traversal | `70.2 ms` | **`306.5 ms`** | `83.3 ms` | `1871.0 ms` |
| `Collections` | **Hash Map** | 20k insertions, updates & lookups | `2.8 ms` | **`7.5 ms`** | `11.3 ms` | `15.7 ms` |
| `Numeric` | **Mandelbrot Fractal** | 200×100 grid, 200 iters | `2.1 ms` | **`6.7 ms`** | `6.4 ms` | `69.2 ms` |
| `Numeric` | **Matrix Multiply** | 120×120 dense integer matrix mult | `1.0 ms` | **`4.0 ms`** | `8.6 ms` | `111.9 ms` |
| `Numeric` | **Monte Carlo Simulation** | 500,000 iterations (Pi approx) | `3.0 ms` | **`7.1 ms`** | `41.0 ms` | `119.8 ms` |
| `Strings` | **FNV-1a String Hash** | 50,000 hash calculations | `3.2 ms` | **`6.0 ms`** | `9.1 ms` | `275.0 ms` |
| `Memory` | **Linked List Allocation** | 50k node alloc & traversal | `1.8 ms` | **`1.7 ms`** | `4.8 ms` | `19.2 ms` |
| `Crypto` | **RC4 Stream Cipher** | 100k bytes KSA & PRGA stream | `0.8 ms` | **`1.6 ms`** | `5.2 ms` | `21.9 ms` |
| `Bitwise` | **Kernighan Popcount** | 100k integers bit-clearing | `1.6 ms` | **`1.9 ms`** | `6.3 ms` | `86.1 ms` |
...
| :--- | :---: | :---: | :---: | :---: |
| **Recursive Fibonacci** | **16.2x faster** | **2.0x faster** | **2.6x** | 🟢 Native Fast |
| **In-Place Quicksort** | **14.1x faster** | `3.1x slower` | **7.1x** | 🟡 Near-Native |
| **Sieve of Eratosthenes** | **10.6x faster** | **3.9x faster** | **1.6x** | 🟢 Near-C |
| **Collatz Conjecture** | **6.5x faster** | `2.1x slower` | **5.0x** | 🟢 Native Fast |
| **Binary Search** | **11.2x faster** | **1.9x faster** | **2.0x** | 🟢 Near-C |
| **Binary Trees** | **6.1x faster** | `3.7x slower` | **4.4x** | 🟢 Native Fast |
| **Hash Map** | **2.1x faster** | **1.5x faster** | **2.6x** | 🟢 Native Fast |
| **Mandelbrot Fractal** | **10.3x faster** | `1.0x slower` | **3.1x** | 🟢 Native Fast |
| **Matrix Multiply** | **28.0x faster** | **2.1x faster** | **4.0x** | 🟢 Native Fast |
| **Monte Carlo Simulation** | **16.9x faster** | **5.8x faster** | **2.4x** | 🟢 Native Fast |
| **FNV-1a String Hash** | **45.7x faster** | **1.5x faster** | **1.9x** | 🟢 Near-C |
| **Linked List Allocation** | **11.2x faster** | **2.8x faster** | **1.0x** | 🟢 Near-C |
| **RC4 Stream Cipher** | **13.9x faster** | **3.3x faster** | **1.9x** | 🟢 Near-C |
| **Kernighan Popcount** | **45.4x faster** | **3.3x faster** | **1.2x** | 🟢 Near-C |
...
* **Result:** **2.6x of C (-O2)**, **2.0x faster than Bun**, and **16.2x faster than Python**.
...
* **Result:** **7.1x of C (-O2)**, **3.1x slower than Bun**, and **14.1x faster than Python**.
...
* **Result:** **1.6x of C (-O2)**, **3.9x faster than Bun**, and **10.6x faster than Python**.
...
* **Result:** **5.0x of C (-O2)**, **2.1x slower than Bun**, and **6.5x faster than Python**.
...
* **Result:** **2.0x of C (-O2)**, **1.9x faster than Bun**, and **11.2x faster than Python**.
...
* **Result:** **4.4x of C (-O2)**, **3.7x slower than Bun**, and **6.1x faster than Python**.
...
* **Result:** **2.6x of C (-O2)**, **1.5x faster than Bun**, and **2.1x faster than Python**.
...
* **Result:** **3.1x of C (-O2)**, **1.0x slower than Bun**, and **10.3x faster than Python**.
...
* **Result:** **4.0x of C (-O2)**, **2.1x faster than Bun**, and **28.0x faster than Python**.
...
* **Result:** **2.4x of C (-O2)**, **5.8x faster than Bun**, and **16.9x faster than Python**.
...
* **Result:** **1.9x of C (-O2)**, **1.5x faster than Bun**, and **45.7x faster than Python**.
...
* **Result:** **1.0x of C (-O2)**, **2.8x faster than Bun**, and **11.2x faster than Python**.
...
* **Result:** **1.9x of C (-O2)**, **3.3x faster than Bun**, and **13.9x faster than Python**.
...
* **Result:** **1.2x of C (-O2)**, **3.3x faster than Bun**, and **45.4x faster than Python**.

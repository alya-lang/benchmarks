...
| :--- | :---: | :---: | :---: | :---: |
| **Geometric Mean Relative Speed** | **1.0x (Baseline)** | `0.39x` *(faster)* | `0.66x` | `0.07x` *(slower)* |
| **Alya Relative Performance** | **Reference Target** | **~2.6x of C** | **1.5x faster** | **14.8x faster** |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| `Algorithms` | **Recursive Fibonacci** | `fib(30)` (~2.69M calls) | `2.4 ms` | **`8.6 ms`** | `12.5 ms` | `120.7 ms` |
| `Algorithms` | **In-Place Quicksort** | 50,000 items in-place sort | `14.7 ms` | **`129.8 ms`** | `32.3 ms` | `1799.6 ms` |
| `Algorithms` | **Sieve of Eratosthenes** | Primes under 50,000 | `1.0 ms` | **`1.6 ms`** | `6.6 ms` | `16.8 ms` |
| `Algorithms` | **Collatz Conjecture** | Under 100k (~2.16M steps) | `14.6 ms` | **`58.5 ms`** | `41.3 ms` | `746.2 ms` |
| `Algorithms` | **Binary Search** | 100k items, 50k lookups | `3.6 ms` | **`6.5 ms`** | `14.2 ms` | `115.5 ms` |
| `Collections` | **Binary Trees** | Heap tree allocation & traversal | `107.5 ms` | **`441.5 ms`** | `96.5 ms` | `2678.0 ms` |
| `Collections` | **Hash Map** | 20k insertions, updates & lookups | `4.3 ms` | **`10.5 ms`** | `15.5 ms` | `23.4 ms` |
| `Numeric` | **Mandelbrot Fractal** | 200×100 grid, 200 iters | `3.2 ms` | **`8.1 ms`** | `9.4 ms` | `130.7 ms` |
| `Numeric` | **Matrix Multiply** | 120×120 dense integer matrix mult | `1.2 ms` | **`7.4 ms`** | `12.6 ms` | `187.0 ms` |
| `Numeric` | **Monte Carlo Simulation** | 500,000 iterations (Pi approx) | `3.7 ms` | **`6.7 ms`** | `54.5 ms` | `190.7 ms` |
| `Strings` | **FNV-1a String Hash** | 50,000 hash calculations | `4.7 ms` | **`8.9 ms`** | `12.7 ms` | `434.0 ms` |
| `Memory` | **Linked List Allocation** | 50k node alloc & traversal | `3.0 ms` | **`2.8 ms`** | `6.9 ms` | `29.1 ms` |
| `Crypto` | **RC4 Stream Cipher** | 100k bytes KSA & PRGA stream | `1.1 ms` | **`2.0 ms`** | `6.9 ms` | `34.0 ms` |
| `Bitwise` | **Kernighan Popcount** | 100k integers bit-clearing | `2.3 ms` | **`3.8 ms`** | `8.8 ms` | `138.3 ms` |
...
| :--- | :---: | :---: | :---: | :---: |
| **Recursive Fibonacci** | **14.1x faster** | **1.5x faster** | **3.6x** | 🟢 Native Fast |
| **In-Place Quicksort** | **13.9x faster** | `4.0x slower` | **8.8x** | 🟡 Near-Native |
| **Sieve of Eratosthenes** | **10.7x faster** | **4.2x faster** | **1.6x** | 🟢 Near-C |
| **Collatz Conjecture** | **12.7x faster** | `1.4x slower` | **4.0x** | 🟢 Native Fast |
| **Binary Search** | **17.7x faster** | **2.2x faster** | **1.8x** | 🟢 Near-C |
| **Binary Trees** | **6.1x faster** | `4.6x slower` | **4.1x** | 🟢 Native Fast |
| **Hash Map** | **2.2x faster** | **1.5x faster** | **2.4x** | 🟢 Native Fast |
| **Mandelbrot Fractal** | **16.1x faster** | **1.2x faster** | **2.6x** | 🟢 Native Fast |
| **Matrix Multiply** | **25.3x faster** | **1.7x faster** | **6.0x** | 🟡 Near-Native |
| **Monte Carlo Simulation** | **28.3x faster** | **8.1x faster** | **1.8x** | 🟢 Near-C |
| **FNV-1a String Hash** | **48.9x faster** | **1.4x faster** | **1.9x** | 🟢 Near-C |
| **Linked List Allocation** | **10.3x faster** | **2.4x faster** | **1.0x** | 🟢 Near-C |
| **RC4 Stream Cipher** | **17.4x faster** | **3.5x faster** | **1.8x** | 🟢 Near-C |
| **Kernighan Popcount** | **36.7x faster** | **2.3x faster** | **1.6x** | 🟢 Near-C |
...
* **Result:** **3.6x of C (-O2)**, **1.5x faster than Bun**, and **14.1x faster than Python**.
...
* **Result:** **8.8x of C (-O2)**, **4.0x slower than Bun**, and **13.9x faster than Python**.
...
* **Result:** **1.6x of C (-O2)**, **4.2x faster than Bun**, and **10.7x faster than Python**.
...
* **Result:** **4.0x of C (-O2)**, **1.4x slower than Bun**, and **12.7x faster than Python**.
...
* **Result:** **1.8x of C (-O2)**, **2.2x faster than Bun**, and **17.7x faster than Python**.
...
* **Result:** **4.1x of C (-O2)**, **4.6x slower than Bun**, and **6.1x faster than Python**.
...
* **Result:** **2.4x of C (-O2)**, **1.5x faster than Bun**, and **2.2x faster than Python**.
...
* **Result:** **2.6x of C (-O2)**, **1.2x faster than Bun**, and **16.1x faster than Python**.
...
* **Result:** **6.0x of C (-O2)**, **1.7x faster than Bun**, and **25.3x faster than Python**.
...
* **Result:** **1.8x of C (-O2)**, **8.1x faster than Bun**, and **28.3x faster than Python**.
...
* **Result:** **1.9x of C (-O2)**, **1.4x faster than Bun**, and **48.9x faster than Python**.
...
* **Result:** **1.0x of C (-O2)**, **2.4x faster than Bun**, and **10.3x faster than Python**.
...
* **Result:** **1.8x of C (-O2)**, **3.5x faster than Bun**, and **17.4x faster than Python**.
...
* **Result:** **1.6x of C (-O2)**, **2.3x faster than Bun**, and **36.7x faster than Python**.
